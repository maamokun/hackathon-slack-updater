...
export async function GET(request: NextRequest) {
  ...
  try {
    console.log(`[Emoji Cron] Starting emoji refresh at ${new Date().toISOString()}`);
    ...
    console.log(`[Emoji Cron] Found ${organizations.length} organizations to refresh`);
    ...
    for (const org of organizations) {
      try {
        ...
        if (org.emojisLastFetched && org.emojisLastFetched > oneHourAgo) {
          console.log(`[Emoji Cron] Skipping ${org.slackTeamName} - updated recently`);
          results.skipped++;
          continue;
        }
        ...
        console.error(`[Emoji Cron] ✗ Failed to refresh emojis for ${org.slackTeamName}:`, error);
      }
    }
